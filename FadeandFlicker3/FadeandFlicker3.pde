...
int fadeValue=0; // brightness value for the analog LEDs (0-255)
unsigned long lastTime = 0;//Timer for blink
unsigned long thisTime = 0;//Timer for right now
unsigned long nextTime = 0;//timer for next event
...
int maxflicker_Longevita =10; // Massimo tempo in millisecondi dura ogni flicker individuale
int minflicker_Longevita = 5; // Minimo tempo in millisecondi dura ogni flicker individuale
int flicker=0; // variable for flickering
int delaySpeed = 30; //Standard Delay
...
void FlickMill()
{
  ...
  thisTime =millis();//right now
  while ((abs(thisTime-lastTime))<(random(5000,10000))) {//do this for 5-10 seconds
    thisTime =millis(); //right now
    if (thisTime > nextTime) {//if the timer has run out change the brightness
      flicker_Longevita = random(minflicker_Longevita,maxflicker_Longevita);//choose a time for the blink
      nextTime = thisTime + flicker_Longevita;//set the time for the blink
      intensita_Luce = random (min_intensita_Luce,max_intensita_Luce); // Intensita luce da random
      analogWrite(ledPin, intensita_Luce);   // accende lo LED  
    }
  }}
