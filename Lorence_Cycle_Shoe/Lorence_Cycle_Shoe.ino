...
void loop() {
  ...
  if (QS == false) {                      // Quantified Self flag is true when arduino finds a heartbeat
    thisBlink = millis();
    while ((millis() - thisBlink) < cadance) { //Fade up: time right now minus time we started is less than 1000
      R = 0;
      G = map((millis() - thisBlink), 0, cadance, 0, 255);
      B = 0;
      strip.setPixelColor(0, strip.Color(R, G, B));
      strip.show();
      
    }
    while ((millis() - thisBlink) < cadance * 2) {//Fade Down
      R = 0;
      G = map((millis() - thisBlink), cadance, cadance * 2, 255, 0);
      B = 0;
      strip.setPixelColor(0, strip.Color(R, G, B));
      strip.show();
    }
