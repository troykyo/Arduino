...
const unsigned int pause = 300;
const int readings = 4;
const int sogliaMaster = 50;
int cuore[] = { 
...
void loop() {
  ...
  for (int k = 1; k < 8; k++)  { 
    ...
    if (average[k] > (sogliaFreq[k]+sogliaAccende[k])){
      bright = map(average[k],sogliaFreq[k],sogliaMax[k],0,255);
...
void recalibrate(){
  ...
  for (int k = 1; k < 8; k++)  { 
    ...
    sogliaAccende[k] = (abs(sogliaMax[k]-sogliaFreq[k]))/sogliaMaster;
    Serial.print(average[k]);
    Serial.print ("\t");
...
void debug(){
      for (int k = 1; k < 16; k++)  {  //reset all the thresholds
        analogWrite (cuore[k], 255);//reset the lights.
  }
  delay (1000);
    for (int k = 1; k < 16; k++)  {  //reset all the thresholds
        analogWrite (cuore[k], 0);//reset the lights.
